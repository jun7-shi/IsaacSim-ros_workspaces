...
ros2 topic hz /clock
ros2 topic hz /odom
ros2 topic hz /g1/head_rgbd/camera_info
...
worktree branch `hum-nav-g1-nav-task`. The current ROS bridge implementation is
commit `38cc08d`.
...
Enable TF/odometry with `--enable_nav_ros_tf_odom`; it creates
`map -> odom -> base_link` and publishes `/odom` through Isaac Sim's built-in
`isaacsim.ros2.bridge`.

Enable the RGBD camera bridge with `--enable_nav_ros_pointcloud`; it creates a
render product for the G1 head RGBD camera and publishes
`/g1/head_rgbd/camera_info` plus `/g1/head_rgbd/points` with `ROS2CameraHelper`
in `depth_pcl` mode.
...
| Sim time | `/clock` | `--enable_nav_ros_clock` creates an official-style ROS2 Bridge clock graph; live `ros2 topic hz /clock` still needs a full sim run | Ready for runtime verification |
...
1. `/clock`, generated by `--enable_nav_ros_clock`.
2. `/odom` and `map -> odom -> base_link`, generated by
   `--enable_nav_ros_tf_odom`.
3. `/g1/head_rgbd/camera_info` plus `/g1/head_rgbd/points` as
   `sensor_msgs/PointCloud2`, generated from the G1 head RGBD camera by
   `--enable_nav_ros_pointcloud`.
...
3. Topic rates for `/clock`, `/odom`, and `/g1/head_rgbd/points`.
